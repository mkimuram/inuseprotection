...
// nodes. The multi-threaded InUseProtector.attemptToDeleteItem() reads the nodes.
...
type node struct {
	...
	// useeReferences with the users recorded in the graph.
	usees []useeref.UseeReference
}
...
func (n *node) clone() *node {
	c := &node{
		...
		virtual:            n.virtual,
		usees:              make([]useeref.UseeReference, 0, len(n.usees)),
	}
	for dep := range n.dependents {
		c.dependents[dep] = struct{}{}
	}
	for _, usee := range n.usees {
		c.usees = append(c.usees, usee)
	}
	return c
}
...
// items could be added to or removed from useeNode.dependents the moment this
// function returns.
...
// n, i.e., the dependent that has a useeReference pointing to n.
// Note that this function does not provide any synchronization guarantees;
// items could be added to or removed from userNode.dependents the moment this
// function returns.
func (n *node) blockingDependents() []*node {
	dependents := n.getDependents()
	var ret []*node
	for _, dep := range dependents {
		for _, usee := range dep.usees {
			if usee.UID == n.identity.UID {
				ret = append(ret, dep)
			}
		}
	}
...
// useeReferenceCoordinates returns a usee reference containing only the coordinate fields
// from the input reference (uid, name, kind, apiVersion)
func useeReferenceCoordinates(ref useeref.UseeReference) useeref.UseeReference {
	return useeref.UseeReference{
		UID:        ref.UID,
		Name:       ref.Name,
		Kind:       ref.Kind,
...
// useeReferenceMatchesCoordinates returns true if all of the coordinate fields match
// between the two references (uid, name, kind, apiVersion)
func useeReferenceMatchesCoordinates(a, b useeref.UseeReference) bool {
